...
export function RoomSection({ room: initialRoom, localPlayer, onLeave }: RoomSectionProps) {
  ...
  const handleDealCards = async () => {
    ...
    await batchUpdatePlayerRoles(updates);
  ...
  const handleRestart = async () => {
    ...
    for (const player of gamePlayers) {
      await updatePlayerRole(player.id, '');
      await updatePlayerAlive(player.id, true);
    }
  ...
  const togglePlayerAlive = async (playerId: string, currentStatus: boolean) => {
    if (!isRoomHost) return;
    await updatePlayerAlive(playerId, !currentStatus);
  ...
  const handleTransferSheriff = async (newSheriffId: string | null) => {
    if (!isRoomHost) return;
    await supabase.from('rooms').update({ sheriff_id: newSheriffId }).eq('id', room.id);
  ...
  const handleTearSheriff = async () => {
    if (!isRoomHost) return;
    await supabase.from('rooms').update({ sheriff_torn: true, sheriff_id: null }).eq('id', room.id);
    setSheriffTorn(true);
    setSheriffId(null);
  };
  ...
  const savePlayerName = async () => {
    ...
    await updatePlayerName(editingPlayerId, editingName.trim());
  ...
  const handlePlayerDie = async (playerId: string) => {
    await updatePlayerAlive(playerId, false);
    loadPlayers();
  };
